...
% P eh o avo de Y, F eh o pai de Y.
bisavo(X,Y):- progenitor(X,P), progenitor(P,F), progenitor(F,Y), homem(X).

% tio(X,Y) /* X é tio de Y */
% Para ser tio, X tem que ser irmao do pai de Y, e X ser homem. 
% F eh o pai de Y, P eh o pai de X e F. 
tio(X,Y):- progenitor(P,X),progenitor(F,Y), progenitor(P,F), homem(X).

% sobrinho(X,Y) /* X é sobrinho de Y */
% Se T eh o pai de X e T eh irmao de Y, e X for homem. 
sobrinho(X,Y):- progenitor(T,X), irmao(T,Y), homem(X).
%sobrinho(X,Y):- progenitor(A,Y), progenitor(A,P), progenitor(P,X), homem(Y).
%testes
test_Pai(x) :- 
