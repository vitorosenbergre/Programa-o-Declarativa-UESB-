...
% Se o elemento procurado pro o primeiro da lista, já aceita. 
...
% Se faz uma analise em todos os elementos da lista para verificação.
...
% Verifica se X e Y são elementos adjacentes em List, verificando os elementos adjacentes da lista 
% e verificando se X e Y se encaixam na adjacencia. Se não encontrar nos primeiros adjacentes,
...
% lista com o X. Vai remover todos os elementos de L1 e fazer a analise. Todos 
...
% vai retornar L2. Numa regra usa o dif para fazer a diferenciação dos elementos e o X e noutra 
% verifica se são iguais. Na que verifica a igualdade, faz o inverso da primeira regra (select).
...
% Ver se um elemento é membro de outra lista (utilizando membro), se for true, então armazena em L3.
% Fez a ordenação e tirou as repeticoes. Apos isso, chamou a regra intersectionAux para verificar os elementos.
% intersectionAux(L1,L2,L3), L1 e L2 são as listas ordenadas sem repeticoes, e L3 é a intesecção entre eles.
...
% Faz a ordenação de L1 e L2, e chama a regra deferenceAux para ver a diferenca entre os conjuntos(listas).
% diferenceAux(L1,L2,L3), L1 e L2 são os conjuntos, e L3 é a diferenca entre eles.
...
% Apos isso chama a regra equivalenceAux(L1,L2), onde vai verificar se eles são equivalente sou não.
